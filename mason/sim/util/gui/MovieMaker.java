    ...
    public synchronized boolean add(BufferedImage image)
        ...
        //              ((sim.util.media.MovieEncoder)encoder).add(image);
                ...
                try  // NOT LIKELY TO HAPPEN
                    {
                    //hai: print to png files                	
                    //encoderClass.getMethod("add", new Class[]{BufferedImage.class}).
                     //   invoke(encoder, new Object[]{image});
                	fileNumber ++; 
                	if(fileNumber >= 0){
                	//String title = "./log/pngs/"+System.currentTimeMillis()+ "_" +System.nanoTime() + ".png";
                    String title = "./log/pngs/"+(int)(fileNumber%100000)/10000+ (int)(fileNumber%10000)/1000 +(int)(fileNumber%1000)/100+(int)(fileNumber%100)/10+(int)fileNumber%10 + ".png";
                    
                	File outputfile = new File(title);
                    ImageIO.write(image, "png", outputfile);
                	}
                    //end of hai's modification
                    }
                catch(Exception ex)
                    {
                    ex.printStackTrace();
