...
public class MovieEncoder implements DataSinkListener, ControllerListener, java.io.Serializable
    ...
    */
    ...
    public static void main(String argc[]){
    	MovieEncoder me = new MovieEncoder();
    	Float fps = Float.valueOf("20").floatValue();
    	Format[] f = me.getEncodingFormats((float)20.0, null);
    ...
    private Format[] getEncodingFormatsHelper(float fps, BufferedImage typicalImage)
        ...
        try
        	...
        	// Get the size
        	width = typicalImage.getWidth(iobs);
        	height = typicalImage.getHeight(iobs);
        	if (width < 1 || height < 1)
        	    return null;
        	scan = (width + 3) & ~3;
        	...
        	pg = new PixelGrabber(typicalImage, 0, 0, width, height,
        			      data, 0, scan);
        	try {
        	    pg.grabPixels();
            ...
            Format format2 = new javax.media.format.JPEGFormat(size, scan*height, Format.byteArray, frameRate, -1, 0);
            
            //Format format3 = new javax.media.format.VideoFormat(encoding, size, maxDataLength, dataType, blueMask)
            //endof hai modification
            ...
            //f[0] = new javax.media.format.JPEGFormat();
    ...
    void setup(BufferedImage i) 
      ...
      //hai modification
    	...
    	// Get the size
    	width = i.getWidth(iobs);
    	height = i.getHeight(iobs);
    	if (width < 1 || height < 1)
    	    return;
    	scan = (width + 3) & ~3;
    	size = new Dimension(width, height);
    	...
    	// Get the pixels into an int array
    	pg = new PixelGrabber(i, 0, 0, width, height,
    			      data, 0, scan);
    	try {
    	    pg.grabPixels();
    	} catch (InterruptedException ie) {
