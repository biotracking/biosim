...
public class TestConfig extends Sim

{
	...
	public int numagents = 1; // ant(predator) number constant
	...
	public TestConfig(long seed) {
		this(seed, 0.5, 0.24);
	}

	public TestConfig(long seed, double width, double height) {
	...
	public void start() {
		...
		for (int i = 0; i < numObstacles; i++) {
			locationX = 0.25;//xMax / 2;// Math.random()*xMax/2 + xMin+0.03;
			locationY = 0.12;//yMax / 2;// Math.random()*yMax/2 + yMin+0.01;
			obs = new ObstacleBody(locationX, locationY, 0.2, i, 3);
			obstacleCounter++;
...
*/
		...
		for (int i = 0; i < numagents; i++) {
			locationX = 0.04;//Math.random() * (xMax - xMax/10) + xMax/20;
			locationY = 0.04;//Math.random() * (yMax - yMax/10) + yMax/20;
			directionX = 1;//Math.random() * 2 - 1;
			directionY = 0;//Math.sqrt(1 - directionX * directionX);
			directionY *= Math.random() > 0.5 ? 1 : -1;
			AntBody agentBody = new AntBody(locationX, locationY, directionX,
					directionY, i, 1, predatorRadius);
			TestController agentC = new TestController(agentBody);
			agentBody.setLogging(true);
			agentBody.setDistanceOfPerception(0.1);
