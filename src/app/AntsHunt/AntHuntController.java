...
public class AntHuntController extends AgentController {

	public double TimeStamp = 0;
	private double[] avoidPara = {1.0,0.9}; // (%) [0] > [1]
	private double noiseTimeOut = 2;
	private int exploreTimeOut = 10;
	private int followTimeOut = 7;
	private int acquireTimeOut = 4;
	...
	private NodeBoolean EXPLORE_STATE_CHECK;
	private NodeBoolean FOLLOW_STATE_CHECK;
	private NodeBoolean ACQUIRE_STATE_CHECK;
	private NodeBoolean SUBDUED_STATE_CHECK;
	private NodeBoolean SEARCH_ANT_STATE_CHECK;
	private NodeBoolean SEARCH_PREY_TIME_STATE_CHECK;
	private NodeBoolean DELIVER_1_STATE_CHECK;
	private NodeBoolean DELIVER_2_STATE_CHECK;
	...
	private String[] stateName={"LOITER","EXPLORE","FOLLOW","ACQUIRE","SUBDUE","SEARCH_ANT",
			"SEARCH_PREY","DELIVER_TO_HOME","DELIVER_TO_RAMDOM_PLACE","RELEASE"};// set up states' name

	public AntHuntController() {
	}

	public AntHuntController(AbstractRobot ab) {
		this.setBody(ab);
		configure(ab);
		clayControl = new ClayController(ab, turret_configuration, steering_configuration, gripper_fingers_configuration, STATE_MACHINE);
	}
	...
	public void configure(AbstractRobot ab) {
		...
		// --- homebase
		PS_HOMEBASE0_GLOBAL = new v_FixedPoint_(ab.getHomeLoc()[0], ab.getHomeLoc()[1]);
...
//		// check state
//		EXPLORE_STATE_CHECK = new b_Equal_i(1,STATE_MACHINE);
//		FOLLOW_STATE_CHECK = new b_Equal_i(2,STATE_MACHINE);
//		ACQUIRE_STATE_CHECK = new b_Equal_i(3,STATE_MACHINE);
//		SUBDUED_STATE_CHECK = new b_Equal_i(4,STATE_MACHINE);
//		SEARCH_ANT_STATE_CHECK = new b_Equal_i(5,STATE_MACHINE);
//		SEARCH_PREY_TIME_STATE_CHECK = new b_Equal_i(6,STATE_MACHINE);
//		DELIVER_1_STATE_CHECK = new b_Equal_i(7,STATE_MACHINE);
//		DELIVER_2_STATE_CHECK = new b_Equal_i(8,STATE_MACHINE);
		...
		ACQUIRE_TIME_OUT = new b_TimeOutCounter_i(acquireTimeOut, ab);
		...
		// follow ant
		MS_FOLLOW_ANT = new v_LinearAttraction_v(ab, 1, 0.0, PS_CLOSEST_ANT);
		// swirl obstacles wrt target 0
		MS_SWIRL_OBSTACLES_PREY = new v_Swirl_vav(1.0, avoidPara[1], PS_OBS, PS_CLOSEST_PREY);
		// swirl obstacles wrt homebase0
		MS_SWIRL_OBSTACLES_HOMEBASE0 = new v_Swirl_vav(1.0, avoidPara[1], PS_OBS, PS_HOMEBASE0);
		// go home 0
		MS_MOVE_TO_HOMEBASE0 = new v_LinearAttraction_v(ab, 1, 0.0, PS_HOMEBASE0);
		// go to target0
		MS_MOVE_TO_PREY = new v_LinearAttraction_v(ab, 1, 0.0, PS_CLOSEST_PREY);
		// noise vector
		MS_NOISE_VECTOR = new v_Noise_(noiseTimeOut, ab);
		// swirl obstacles wrt noise
		MS_SWIRL_OBSTACLES_NOISE = new v_Swirl_vav(1.0, avoidPara[1], PS_OBS, MS_NOISE_VECTOR);
		// go to subdue target
		// MS_MOVE_TO_SUBDUE_TARGET = new
		// v_LinearAttraction_v(ab,1,0.0,PS_SUBDUED_PREY);
		// not move
		MS_NOT_MOVE = new v_Stop_r();
		// spiral
		MS_SPIRAL = new v_Spiral_r(SEE_PREY);
		// swirl obstacles wrt noise
		MS_SWIRL_OBSTACLES_SPIRAL = new v_Swirl_vav(1.0, avoidPara[1], PS_OBS, MS_SPIRAL);
		...
		AS_LOITER.weights[1] = 0.0001;
		AS_LOITER.embedded[1] = MS_NOISE_VECTOR;

		AS_LOITER.weights[2] = 0.001;
		AS_LOITER.embedded[2] = MS_AVOID_OBSTACLES;

		AS_LOITER.weights[3] = 0.001;
		...
		AS_EXPLORE.weights[0] = 1.0;
		AS_EXPLORE.embedded[0] = MS_AVOID_OBSTACLES;

		AS_EXPLORE.weights[1] = 0.6;
		...
		AS_DELIVER_TO_HOME.weights[1] = 0.6;
		...
		AS_DELIVER_TO_RANDOM_PLACE.weights[3] = 0.8;
		AS_DELIVER_TO_RANDOM_PLACE.embedded[3] = MS_AVOID_ANTS;

		AS_DELIVER_TO_RANDOM_PLACE.weights[4] = 0.6;
		...
		STATE_MACHINE.follow_on[stateNum][0] = stateNum + 1; // transition to AS_EXPLORE
		STATE_MACHINE.triggers[stateNum][1] = IS_BUMPED;
		STATE_MACHINE.follow_on[stateNum][1] = stateNum + 2; // transition to AS_EXPLORE
		STATE_MACHINE.triggers[stateNum][2] = SEE_PREY;
		STATE_MACHINE.follow_on[stateNum][2] = stateNum + 3; // transition to ACQUIRE
		stateNum++;
		// STATE 1 AS_EXPLORE
		STATE_MACHINE.triggers[stateNum][0] = SEE_PREY;
		STATE_MACHINE.follow_on[stateNum][0] = stateNum + 2; // transition to ACQUIRE
		STATE_MACHINE.triggers[stateNum][1] = EXPLORE_TIME_OUT;
		STATE_MACHINE.follow_on[stateNum][1] = stateNum-1;
		STATE_MACHINE.triggers[stateNum][2] = IS_BUMPED;
		STATE_MACHINE.follow_on[stateNum][2] = stateNum + 1; // transition to AS_EXPLORE
		stateNum++;
		// STATE 2 AS_FOLLOW
		STATE_MACHINE.triggers[stateNum][0] = SEE_PREY;
		STATE_MACHINE.follow_on[stateNum][0] = stateNum + 1; // transition to ACQUIRE
		...
		STATE_MACHINE.follow_on[stateNum][2] = stateNum + 3; // transition to SEARCH ANT
		stateNum++;
		// STATE 3 ACQUIRE
		STATE_MACHINE.triggers[stateNum][0] = CLOSE_TO_PREY; // TARGET0_IN_GRIPPER;
		STATE_MACHINE.follow_on[stateNum][0] = stateNum + 1; // transition to SUBDUE
		STATE_MACHINE.triggers[stateNum][1] = NOT_PREY_VISIBLE;
		STATE_MACHINE.follow_on[stateNum][1] = stateNum - 2; // transition to WANDER
		STATE_MACHINE.triggers[stateNum][2] = ACQUIRE_TIME_OUT;
		STATE_MACHINE.follow_on[stateNum][2] = stateNum - 2; // transition to WANDER
		stateNum++;
		// STATE 4 SUBDUE
		STATE_MACHINE.triggers[stateNum][0] = PREY_IN_GRIPPER; // SUBDUED_PREY;
		STATE_MACHINE.follow_on[stateNum][0] = stateNum + 3; // transition to AS_DELIVER TO HOME
		STATE_MACHINE.triggers[stateNum][1] = SUBDUED_TIME_OUT;
		STATE_MACHINE.follow_on[stateNum][1] = stateNum + 2; // transition to SEARCH PREY
		STATE_MACHINE.triggers[stateNum][2] = NOT_CLOSE_TO_PREY;
		STATE_MACHINE.follow_on[stateNum][2] = stateNum + 2; // transition to SEARCH PREY
		stateNum++;
		// STATE 5(5.1) SEARCH ANT
		STATE_MACHINE.triggers[stateNum][0] = SEE_PREY;
		STATE_MACHINE.follow_on[stateNum][0] = stateNum - 2; // transition to ACQUIRE
		STATE_MACHINE.triggers[stateNum][1] = SEARCH_ANT_TIME_OUT;
		STATE_MACHINE.follow_on[stateNum][1] = stateNum - 4; // transition to EXPLORE
		STATE_MACHINE.triggers[stateNum][2] = SEE_ANT;
		STATE_MACHINE.follow_on[stateNum][2] = stateNum - 3; // transition to FOLLOW
		stateNum++;
		// STATE 6(5.2) SEARCH PREY
		STATE_MACHINE.triggers[stateNum][0] = SEE_PREY;
		STATE_MACHINE.follow_on[stateNum][0] = stateNum - 3; // transition to ACQUIRE
		STATE_MACHINE.triggers[stateNum][1] = SEARCH_PREY_TIME_OUT;
		STATE_MACHINE.follow_on[stateNum][1] = stateNum - 5; // transition to EXPLORE
		stateNum++;
		// STATE 7(6.1) DELIVER TO HOME
		STATE_MACHINE.triggers[stateNum][0] = CLOSE_TO_HOMEBASE0;
		STATE_MACHINE.follow_on[stateNum][0] = stateNum + 2; // transition to DROP IN HOME
		STATE_MACHINE.triggers[stateNum][1] = PREY_NOT_IN_GRIPPER;
		STATE_MACHINE.follow_on[stateNum][1] = stateNum - 1; // transition to SEARCH PREY
		STATE_MACHINE.triggers[stateNum][2] = DELIVER_1_TIME_OUT;
		STATE_MACHINE.follow_on[stateNum][2] = stateNum + 1; // transition to DELIVER TO RANDOM PLACE
		stateNum++;
		// STATE 8(6.2) DELIVER TO RANDOM PLACE
		STATE_MACHINE.triggers[stateNum][0] = CLOSE_TO_HOMEBASE0;
		STATE_MACHINE.follow_on[stateNum][0] = stateNum + 1; // transition to DROP IN HOME
		STATE_MACHINE.triggers[stateNum][1] = PREY_NOT_IN_GRIPPER;
		STATE_MACHINE.follow_on[stateNum][1] = stateNum - 2; // transition to SEARCH PREY
		STATE_MACHINE.triggers[stateNum][2] = DELIVER_2_TIME_OUT;
		STATE_MACHINE.follow_on[stateNum][2] = stateNum - 1; // transition to DELIVER TO HOME
		stateNum++;
		// STATE 9(7) DROP IN HOME
		STATE_MACHINE.triggers[stateNum][0] = PREY_NOT_IN_GRIPPER;
		STATE_MACHINE.follow_on[stateNum][0] = stateNum - 8; // transition to EXPLORE
		stateNum++;
		// */
		...
		GRIPPER_FINGERS.embedded[stateNum++] = 0; // close in AS_DELIVER_TO_RANDOM_PLACE
