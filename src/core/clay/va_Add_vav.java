...
/**
 * Add one vector to an array of others. Useful for converting ego to global or
 * global to ego positions for an array of sensor readings.
 * <P>
...
public class va_Add_vav extends NodeVec2Array {
	 ...
	 * 
	 * @param im1
	 *            NodeVec2Array, the node that generates an Array of vectors to
	 *            be added.
	 * @param im2
	 *            NodeVec2, the node that generates a vector to be added to the
	 *            others.
	 */
	public va_Add_vav(NodeVec2Array im1, NodeVec2 im2) {
		if (DEBUG)
			System.out.println("va_Add_vav: instantiated.");
		embedded1 = im1;
		embedded2 = im2;
	}
	...
	public void setembedded(NodeVec2Array im1, NodeVec2 im2) {
		embedded1 = im1;
		embedded2 = im2;
	}
	...
	/**
	 * Return a Vec2[] that is the sum of the original values and the embedded
	 * node.
	 * 
	 * @param timestamp
	...
	public Vec2[] Value(double timestamp) {
		if (DEBUG)
			System.out.println("va_Add_vav: Value()");
		...
		if ((timestamp > lasttime) || (timestamp == -1)) {
			...
			for (int i = 0; i < last_val.length; i++) {
				if (last_val[i] != null) {
					// System.out.print("va_Add_vav->"+last_val[i].x +
					// "\t"+last_val[i].y+"\n");
					last_val[i] = new Vec2(tmp.x + last_val[i].x, tmp.y
							+ last_val[i].y);
				}
			}
		}
