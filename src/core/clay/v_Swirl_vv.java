 ...
 * One embedded node provides a hazard to avoid, the other points in the
...
public class v_Swirl_vv extends NodeVec2 {
	 ...
	 * @param im1
	 *            double, the embedded perceptual schema that generates an item
	 *            to avoid.
	 * @param im2
	 *            double, the embedded perceptual schema that generates a
	 *            pointer to the goal (must be egocentric).
	 */
	public v_Swirl_vv(double soe, double s, NodeVec2 im1, NodeVec2 im2) {
		if (DEBUG)
			System.out.println("v_Swirl_vv: instantiated.");
		embedded1 = im1;
		embedded2 = im2;
		if ((soe < s) || (soe < 0) || (s < 0)) {
	...
	public Vec2 Value(double timestamp) {
		...
		if ((timestamp > lasttime) || (timestamp == -1)) {
			...
			// in front of us.
			tempdir = -Units.BestTurnRad(refheading, obs.t);
			if (Math.abs(tempdir) < Math.PI / 2) {
				...
				// in controlled zone
					...
					tempmag = (sphere - obs.r) / (sphere - safety);
			...
			// last_val.setr(0.0);
			if (DEBUG)
				System.out.println("v_Swirl_vv.Value: " + "output " + last_val);
		}
