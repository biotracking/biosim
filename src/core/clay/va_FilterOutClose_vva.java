...
/**
 * Remove Vec2s that are close to a location from a list of Vec2s. This was used
 * in the Find Life on Mars Competition to ignore attractor objects close to the
 * door of a bin.
 * <P>
...
public class va_FilterOutClose_vva extends NodeVec2Array {
	 ...
	 * 
	 * @param r
	 *            how close things must be to be ignored.
	 * @param im1
	 *            NodeVec2, the embedded node that points to the location to
	 *            filter on.
	 * @param im2
	 *            NodeVec2Arry, the embedded node that generates a list of items
	 *            to filter.
	 */
	public va_FilterOutClose_vva(double r, NodeVec2 im1, NodeVec2Array im2) {
		if (DEBUG)
			System.out.println("va_FilterOutClose_vva: instantiated.");
		embedded1 = im1;
		embedded2 = im2;
		range = r;
	}

	public va_FilterOutClose_vva() {
	}
