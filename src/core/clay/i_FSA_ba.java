...
/**
 * A Finite State Automoton that generates an integer output. While in the
 * current state, if trigger[current_state][edge].Value() is true, the new state
 * is follow_on[current_state][edge]. You must fill in the values of these
 * arrays yourself. It is best to look at an example.
 * <P>
 * This node may be used to implement "Temporal Sequencing" as developed in the
 * Mobile Robot Lab at Georgia Tech.
 * <P>
...
public class i_FSA_ba extends NodeInt {
	...
	/**
	 * Instantiate an FSA. Configuration is by setting trigger and follow_on
	 * arrays.
	 */
		...
		if ((timestamp != lasttime) || (timestamp == -1)) {
			...
			while (triggers[state][i] != null) {
				if (triggers[state][i].booleanValue(timestamp) == true) {
					...
					// System.out.print("i_FSA_ba-> state: "+agentBody.getAgentType()+"\t"+state+"\n");
					break;
				}
				i++;
			}
