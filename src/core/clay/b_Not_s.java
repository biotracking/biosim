...
/**
 * Provides the inverse of an embedded scalar node's value. If the embedded node
 * is not boolean (double or int), then it is considered true if non-zero.
 * <P>
...
public class b_Not_s extends NodeBoolean {
	 ...
	 * 
	 * @param im1
	 *            NodeScalar, the embedded perceptual schema that generates a
	 *            value to be inverted.
	 */
	public b_Not_s(NodeScalar im1) {
		if (DEBUG)
			System.out.println("b_Not_s: instantiated.");
		embedded1 = im1;
	}
	...
	/**
	 * Return a boolean representing the inverse of the output of the embedded
	 * schema.
	 * 
	 * @param timestamp
	...
	public boolean Value(double timestamp) {
		if (DEBUG)
			System.out.println("b_Not_s: Value()");
