...
/**
 * Select one from an array of doubles based on the output of an embedded
 * integer schema. The embedded selector outputs an integer, which indicates
 * which of the doubles to output.
 * <P>
...
public class d_Select_i extends NodeDouble {
	...
	public d_Select_i(NodeInt s) {
		if (DEBUG)
			System.out.println("d_Select_i: instantiated.");
		selector = s;
	}
	...
	/**
	 * Return a double selectively activated based on the embedded NodeInt
	 * selector.
	 * 
	 * @param timestamp
	...
	public double Value(double timestamp) {
		if (DEBUG)
			System.out.println("d_Select_i: Value()");
		...
		if ((timestamp > lasttime) || (timestamp == -1)) {
			...
			if (choice < embedded.length) {
				...
				System.out.println("d_Select_i: illegal" + " selection: "
						+ choice);
			}
		}
