...
public class ElkBody extends AgentPortrayal {
	private double distanceOfPerception = 50; // the distance of elk detect.
	public final double MAX_TRANSLATION = 1; // maximum translation speed
	public double probabilityOfSubdued = 0.8;
	...
	public ElkBody(double x, double y, double d_x, double d_y, int i, int t,
			double size) {
	...
	public void updatePerception(SimState state) {
		...
		perception = ((Sim) state).forest.getObjectsExactlyWithinDistance(
				(new Double2D(loc.x * this.getScaler(), loc.y
						* this.getScaler())), 0.019 * this.getScaler());
	...
	public void takeStep(final SimState state) {
		...
		if (this.getTagged()) {// && this.getSubdued()){
			MutableDouble2D newPos = getPosFromTagger(getTaggerId(),
					(Sim) state);
			loc.x = newPos.x;
			loc.y = newPos.y;
...
//			else
	...
	private boolean checkCollision(double xo, double yo) {
		...
		tempX += xo;
		tempY += yo;
		return true;//(0.004 < tempX && tempX < 0.16 && 0.004 < tempY && tempY < 0.115); // 0929
	}
