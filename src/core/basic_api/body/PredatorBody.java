...
public class PredatorBody extends AgentPortrayal {
	...
	public PredatorBody(double x, double y, double d_x, double d_y, int i, int t,
			double size) {
	...
	public void releaseToHome(int type) {
		if (!getTagging())
			return;
		// System.out.print("predatorBody-> releaseToHome taggeeID= "+getTaggeeId()+"\n");
	...
	public void releaseInLoc(int type) {
		if (!getTagging())
			return;
		// System.out.print("predatorBody-> releaseInLoc taggeeID= "+getTaggeeId()+"\n");
		for (int i = 0; i < perception.numObjs; i++) {
...
//					if (Math.abs(x - xOff) < 0.001 && Math.abs(y - yOff) < 0.001) {
	...
	public void takeAct() {
		...
		if(!this.usedSetVelocities){
			...
			//if(this.getDir().y < 0) dsrTurnRate *= -1; 
		...
		for (int i = 0; i < perception.numObjs; i++) {// choose other predators
			...
			if (tmp.getAgentType() == 2 && tmp.getAlive()) {
				distanceToAgent = tmp.getLoc().distance(loc.x, loc.y);
				if(distanceToAgent < this.getAgentSize()/2 + tmp.getAgentSize()/2){//near to catch
					tmp.setAlive(false);
					System.out.println("Predator " + this.getAgentId() + 
							" caught Prey " + tmp.getAgentId() + " @ " + this.getTimeStamp());
				}
			}
		}
	}
