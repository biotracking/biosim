...
public class DialogMessageJoke extends Dialog implements ActionListener,
		MouseMotionListener {
	private Button ok;
	private int buttonx;
	private TextArea ta;
	 ...
	 * 
	 * @param parent
	 *            Frame, the parent of the box.
	 * @param title
	 *            String, what to put in the title bar.
	 * @param msg
	 *            String, what to say.
	 */
	public DialogMessageJoke(Frame par, String title, String msg) {
		...
		ta.setText(msg);
		this.add(ta);
		ta.setBounds(getInsets().left, getInsets().top, 400 - getInsets().right
				- getInsets().left, 200 - getInsets().top - getInsets().bottom);
		ok = new Button("OK");
	...
	public void addNotify() {
		...
		if (ta != null)
			ta.setBounds(getInsets().left, getInsets().top, 400
					- getInsets().right - getInsets().left, 200
