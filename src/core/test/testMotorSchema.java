...
public class testMotorSchema extends AgentController {

	public long TimeStamp = 0;
	private double[] avoidPara = { 0.8, 0.6 }; // (%) [0] > [1]
	private double noiseTimeOut = 5;
	private int exploreTimeOut = 15;
	private int followTimeOut = 12;
	...
	// private NodeBoolean PF_IS_FELLOW_ANT;
	...
	private NodeVec2 MS_FALLOW_ANT;
	...
	private v_StaticWeightedSum_va AS_DELIVER_TO_RAMDOM_PLACE;
	private v_StaticWeightedSum_va AS_RELEASE;
	private i_FSA_ba STATE_MACHINE;

	public testMotorSchema() {
	}

	public testMotorSchema(AbstractRobot ab) {
		this.setBody(ab);
		configure(ab);
	...
	public void takeStep() {
		...
		this.getFinalTurnRate(allTurnRate);
	}
	...
	public void configure(AbstractRobot ab) {
		...
		EXPLORE_TIME_OUT = new b_TimeOutCounter_i(exploreTimeOut, SEE_PREY, ab);
		...
		// --- set subdued time out and give up
		SUBDUED_TIME_OUT = new b_TimeOutCounter_i(subdueTimeOut,
				PREY_NOT_IN_GRIPPER, ab);
		...
		// bumped time out
		FOLLOW_TIME_OUT = new b_TimeOutCounter_i(followTimeOut, IS_BUMPED, ab);
		// is something in the gripper?
		// TARGET1_IN_GRIPPER = new b_Equal_i(1,PS_IN_GRIPPER); //ant
		// is fellow ant?
		// IS_FELLOW_ANT = new b_Equal_i(1,PS_IN_GRIPPER); // fellow ant type 1
		// is enemy ant?
		...
		// is subdued give up?
		IS_SUBDUED_GIVE_UP = new b_Equal_br(PREY_NOT_IN_GRIPPER,
				SUBDUED_TIME_OUT, ab);
		// is drop?
		...
		SEARCH_ANT_TIME_OUT = new b_TimeOutCounter_i(searchAntTimeOut, SEE_ANT,
				ab);

		SEARCH_PREY_TIME_OUT = new b_TimeOutCounter_i(searchPreyTimeOut,
				SEE_PREY, ab);

		DELIVER_1_TIME_OUT = new b_TimeOutCounter_i(deliver1TimeOut,
				PREY_IN_GRIPPER, ab);

		DELIVER_2_TIME_OUT = new b_TimeOutCounter_i(deliver2TimeOut,
				PREY_IN_GRIPPER, ab);
		...
		// ======
		// avoid obstacles
		MS_AVOID_OBSTACLES = new v_Avoid_va(avoidPara[0], avoidPara[1], PS_OBS);
		// avoid ants
		MS_AVOID_ANTS = new v_Avoid_va(avoidPara[0], avoidPara[1],
				PS_OBS_OF_ANT);
		// fallow ant
		MS_FALLOW_ANT = new v_LinearAttraction_v(ab, 1, 0.0, PS_CLOSEST_ANT);
		...
		MS_SWIRL_OBSTACLES_HOMEBASE0 = new v_Swirl_vav(1.0, avoidPara[1],
				PS_OBS, PS_HOMEBASE0);
		// go home 0
		MS_MOVE_TO_HOMEBASE0 = new v_LinearAttraction_v(ab, 1, 0.0,
				PS_HOMEBASE0);
		// go to target0
		...
		AS_FOLLOW.embedded[2] = MS_FALLOW_ANT;
		...
		// ======
		// AS_DELIVER_TO_RAMDOM_PLACE
		// ======
		AS_DELIVER_TO_RAMDOM_PLACE = new v_StaticWeightedSum_va();
		AS_DELIVER_TO_RAMDOM_PLACE.weights[0] = 1.0;
		AS_DELIVER_TO_RAMDOM_PLACE.embedded[0] = MS_AVOID_OBSTACLES;

		AS_DELIVER_TO_RAMDOM_PLACE.weights[1] = 0.2;
		AS_DELIVER_TO_RAMDOM_PLACE.embedded[1] = MS_MOVE_TO_HOMEBASE0;

		AS_DELIVER_TO_RAMDOM_PLACE.weights[2] = 0.2;
		AS_DELIVER_TO_RAMDOM_PLACE.embedded[2] = MS_SWIRL_OBSTACLES_HOMEBASE0;

		AS_DELIVER_TO_RAMDOM_PLACE.weights[3] = 1.0;
		AS_DELIVER_TO_RAMDOM_PLACE.embedded[3] = MS_AVOID_ANTS;

		AS_DELIVER_TO_RAMDOM_PLACE.weights[4] = 1.0;
		AS_DELIVER_TO_RAMDOM_PLACE.embedded[4] = MS_NOISE_VECTOR;// */
		// ======
		// AS_RELEASE
		// ======
		...
		// RAMDOM PLACE
		stateNum++;
		// STATE 8(6.2) DELIVER TO RAMDOM PLACE
		STATE_MACHINE.triggers[stateNum][0] = CLOSE_TO_HOMEBASE0;
		STATE_MACHINE.follow_on[stateNum][0] = stateNum + 1; // transition to
		// DROP IN HOME
		STATE_MACHINE.triggers[stateNum][1] = PREY_NOT_IN_GRIPPER;
		...
		STEERING.embedded[stateNum++] = AS_DELIVER_TO_RAMDOM_PLACE;
		...
		TURRET.embedded[stateNum++] = AS_DELIVER_TO_RAMDOM_PLACE;
